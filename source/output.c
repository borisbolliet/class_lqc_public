...
int output_init(
  ...
  /** - deal with background quantities */
   ...
   if (pop->write_background_lqc == _TRUE_) {

    class_call(output_background_lqc(pba_lqc,pop),
               pop->error_message,
               pop->error_message);
  ...
  if (pop->write_perturbations_lqc == _TRUE_) {
    ...
    class_call(output_perturbations_lqc(pba_lqc,ppt_lqc,pop),
               pop->error_message,
               pop->error_message);
...
int output_background_lqc(
  ...
  class_call(background_lqc_output_titles(pba,titles),
             pba->error_message,
             pop->error_message);
  number_of_titles = get_number_of_titles(titles);
  size_data = number_of_titles*pba->bt_size;
  class_alloc(data,sizeof(double)*size_data,pop->error_message);
  class_call(background_lqc_output_data(pba,
                                    number_of_titles,
                                    data),
             pba->error_message,
             pop->error_message);

  sprintf(file_name,"%s_%s",pop->root,"background_lqc.dat");
...
int output_perturbations_lqc(
  ...
  for (index_ikout=0; index_ikout<ppt->k_output_values_num; index_ikout++){
    ...
    if (ppt->has_scalars == _TRUE_){
      index_md = ppt->index_md_scalars;
      k = ppt->k[index_md][ppt->index_k_output_values[index_md*ppt->k_output_values_num+index_ikout]];
      sprintf(file_name,"%s%s%d%s",pop->root,"perturbations_lqc_k",index_ikout,"_s.dat");
      class_open(out, file_name, "w", ppt->error_message);
      fprintf(out,"#scalar perturbations for mode k = %.*e Mpc^(-1)\n",_OUTPUTPRECISION_,k);
      output_print_data(out,
                        ppt->scalar_titles,
                        ppt->scalar_perturbations_lqc_data[index_ikout],
                        ppt->size_scalar_perturbation_lqc_data[index_ikout]);
    ...
    if (ppt->has_tensors == _TRUE_){
      index_md = ppt->index_md_tensors;
      k = ppt->k[index_md][ppt->index_k_output_values[index_md*ppt->k_output_values_num+index_ikout]];
      sprintf(file_name,"%s%s%d%s",pop->root,"perturbations_lqc_k",index_ikout,"_t.dat");
      class_open(out, file_name, "w", ppt->error_message);
      fprintf(out,"#tensor perturbations for mode k = %.*e Mpc^(-1)\n",_OUTPUTPRECISION_,k);
      output_print_data(out,
                        ppt->tensor_titles,
                        ppt->tensor_perturbations_lqc_data[index_ikout],
                        ppt->size_tensor_perturbation_lqc_data[index_ikout]);
