...
int input_init_from_arguments(
                              ...
                              struct background_lqc *pba_lqc,
                              struct perturbs_lqc *ppt_lqc,
...
int input_read_parameters(
  ...
  class_call(input_default_params(pba_lqc,ppt_lqc,
                                  pba,
                                  pth,
                                  ppt,
                                  ptr,
  ...
  /** (a) LQC parameters */
  class_read_double("a_bounce",pba_lqc->a_bounce);
  class_read_double("rho_bounce",pba_lqc->rho_bounce);
  //pba_lqc->rho_bounce = 4119702600.e-10*pow(sqrt(8.*_PI_),4.);
//220517    pba_lqc->rho_bounce = 1.;

  class_read_double("N_exit",pba_lqc->N_star);
  class_read_double("x_ini_lqc",pba_lqc->x_ini);
  class_read_double("phi_ini_lqc",pba_lqc->phi_ini);
  class_read_double("phi_dot_ini_lqc",pba_lqc->phi_dot_ini);

  class_read_double("m_scf_lqc",pba_lqc->m_scf_lqc);
  // pba_lqc->m_scf_lqc = sqrt(4.10457e-11);
  class_read_double("Omega0_scf",pba_lqc->Omega0_scf);
  
  class_read_int("has_lqc",pba_lqc->has_lqc);
  class_read_int("has_contracting_phase_lqc",pba_lqc->has_contracting_phase);
  
  class_read_int("has_perturbations_lqc",ppt_lqc->has_perturbations_lqc);
  pba_lqc->has_perturbations_lqc = ppt_lqc->has_perturbations_lqc;
  class_read_int("has_pk_lqc",ppt_lqc->has_pk_lqc);
  class_read_int("has_scalars_lqc",ppt_lqc->has_scalars);
  class_read_int("has_tensors_lqc", ppt_lqc->has_tensors);
  class_read_int("tensors_only", ppt_lqc->tensors_only);
  class_read_int("has_bispectrum_lqc", ppt_lqc->has_bispectrum_lqc);

  class_read_double("alpha_max_for_pk_lqc",pba_lqc->alpha_max_for_pk_lqc);
  class_read_double("alpha_min_for_pk_lqc",pba_lqc->alpha_min_for_pk_lqc);
  class_read_double("ALPHA_end",pba_lqc->ALPHA_end);
  class_read_double("k_max_for_pk_lqc",ppt_lqc->k_max_for_pk);
  class_read_double("k_min_for_pk_lqc",ppt_lqc->k_min_for_pk);
  class_read_double("k_per_decade_for_pk_lqc",ppt_lqc->k_per_decade_for_pk_lqc);

  class_read_double("lambda_k_2",ppt_lqc->lambda_k_2);
  class_read_double("lambda_k_3",ppt_lqc->lambda_k_3);
  ppt_lqc->lambda_k_2 = 1./ ppt_lqc->lambda_k_2;
  ppt_lqc->lambda_k_3 = 1./ ppt_lqc->lambda_k_3;
  pba_lqc->lambda_k_2 = ppt_lqc->lambda_k_2;
  pba_lqc->lambda_k_3 = ppt_lqc->lambda_k_3;
  
  class_read_double("tau_ini_ppt_lqc",ppt_lqc->tau_ini);
  class_read_double("tau_end_ppt_lqc",ppt_lqc->tau_end);
  class_read_double("delta_cut_off_bispectrum",ppt_lqc->delta_cut_off_bispectrum);
  class_read_double("integration_depth_ini",ppt_lqc->integration_depth_ini);
  class_read_double("integration_depth_end",ppt_lqc->integration_depth_end);


  pba_lqc->k_min_for_pk = 1.e-10*pba_lqc->a_bounce*sqrt(3.*pba_lqc->rho_bounce);
//  pba_lqc->m_scf_lqc=2.*_PI_*sqrt(6.*pba_lqc->A_s)/(2.*pba_lqc->N_star+1.);//100817
  pba_lqc->Gamma=sqrt(3.*pba_lqc->rho_bounce)/pba_lqc->m_scf_lqc;
  pba_lqc->H_max=sqrt(3.*pba_lqc->rho_bounce);
  pba_lqc->rho_ini=pba_lqc->rho_bounce;
  pba_lqc->integration_depth_ini = ppt_lqc->integration_depth_ini;
  ...
  */
  ...
  class_read_string("root",pba_lqc->root);
  class_read_string("root",ppt_lqc->root);
  ...
  class_read_int("perturbations_lqc_verbose",
                 ppt_lqc->perturbations_lqc_verbose);
  ...
  class_read_double("back_lqc_integration_stepsize",ppr->back_lqc_integration_stepsize);
  class_read_double("tol_background_lqc_integration",ppr->tol_background_lqc_integration);
  ...
  class_read_double("perturb_lqc_integration_stepsize",ppr->perturb_lqc_integration_stepsize);
  ppt_lqc->tol=ppr->tol_perturb_lqc_integration;

  
  class_read_double("perturb_lqc_sampling_stepsize",ppr->perturb_lqc_sampling_stepsize);
  class_read_double("tol_tau_approx",ppr->tol_tau_approx);
  class_read_double("tol_perturb_integration",ppr->tol_perturb_integration);
  class_read_double("tol_perturb_lqc_integration",ppr->tol_perturb_lqc_integration);
  ...
  if (flag1 == _TRUE_) {
    ...
    qsort (ppt_lqc->k_output_values, ppt_lqc->k_output_values_num, sizeof(double), compare_doubles);
    
    ppt->store_perturbations = _TRUE_;
    ppt_lqc->store_perturbations = _TRUE_;
    pop->write_perturbations = _TRUE_;
    pop->write_perturbations_lqc = _TRUE_;
  }
  ...
  if (flag1 == _TRUE_) {
    ...
    class_test(int1 > _MAX_NUMBER_OF_K_FILES_,
               ...
               "you want to write some output for %d different values of k, hence you should increase _MAX_NUMBER_OF_K_FILES_ in include/perturbations.h to at least this number",
               int1);
  ppt_lqc->k_output_values_num = int1;

    for (i=0; i<int1; i++) {
      ppt_lqc->k_output_values[i] = pointer1[i];
...
int input_default_params(
  ...
  /** - background_lqc structure */

  //pba_lqc->a_today = 2.;
   pba_lqc->alpha_min_for_pk_lqc = 1.e-1;
  pba_lqc->alpha_max_for_pk_lqc = 1.e1;
  pba_lqc->has_contracting_phase = _TRUE_;
  pba_lqc->post_bounce = _TRUE_;
  pba_lqc->remote_past = _FALSE_;
  pba_lqc->a_bounce = 1.;
  pba_lqc->flag_start_inflation= _FALSE_;
  pba_lqc->flag_start_deflation= _FALSE_;
  pba_lqc->flag_end_inflation= _FALSE_;
  pba_lqc->flag_end_deflation= _FALSE_;
  //pba_lqc->rho_bounce = 4119702600.e-10*pow(sqrt(8.*_PI_),4.);
  pba_lqc->Delta0_lqc = 5.17;
  pba_lqc->l0_lqc = //sqrt(3./(0.41*8.*8.*_PI_*_PI_));
	//sqrt(pow(pba_lqc->Delta0_lqc,3.)/(48.*_PI_*_PI_))/sqrt(8.*_PI_);
	sqrt(3.);
  pba_lqc->rho_bounce = 3./pow(pba_lqc->l0_lqc,2.);
  pba_lqc->rho_ini =  pba_lqc->rho_bounce;
  pba_lqc->N_star = 56.;//100817
  pba_lqc->k_star = 0.002; //in Mpc^-1.
  pba_lqc->A_s = 1.e-10*exp(3.094)*pow(0.002/0.05,0.9645-1.);
  pba_lqc->m_scf_lqc=2.*_PI_*sqrt(6.*1.e-10*exp(3.094))/(2.*pba_lqc->N_star+1.);//100817
  pba_lqc->Omega0_scf=1.;
  pba_lqc->x_ini=3.e-5; 
  pba_lqc->phi_ini=7.10065387;
  pba_lqc->phi_dot_ini=1.414213562;

  pba_lqc->tau_end=1.;
  pba_lqc->Gamma=sqrt(3.*pba_lqc->rho_bounce)/pba_lqc->m_scf_lqc;
  pba_lqc->ALPHA_end=10.;
  pba_lqc->ALPHA_ini=10.;
  pba_lqc->integration_depth_ini=500.;

  
  pop->write_background_lqc = _TRUE_;

  /** - perturbations_lqc structure */
  ppt_lqc->perturbations_lqc_verbose = 10;
  ppt_lqc->has_perturbations_lqc = _TRUE_;
  pba_lqc->has_perturbations_lqc = ppt_lqc->has_perturbations_lqc;
  ppt_lqc->integration_depth_ini=pba_lqc->integration_depth_ini;

  ppt_lqc->has_scalars = _TRUE_;
  ppt_lqc->has_tensors = _TRUE_;

  ppt_lqc->has_ad=_TRUE_;
  
  ppt_lqc->has_pk_lqc=_TRUE_;
  ppt_lqc->tensors_only=_FALSE_;
  ppt_lqc->has_bispectrum_lqc=_TRUE_;

  ppt_lqc->k_output_values_num=0;
  //ppt_lqc->store_perturbations = _FALSE_;
  ppt_lqc->number_of_scalar_titles=0;
  ppt_lqc->number_of_tensor_titles=0;
  for (filenum = 0; filenum<_MAX_NUMBER_OF_K_FILES_; filenum++){
    ppt_lqc->scalar_perturbations_lqc_data[filenum] = NULL;
    ppt_lqc->tensor_perturbations_lqc_data[filenum] = NULL;
  }
  ppt_lqc->index_k_output_values=NULL;

  //ppt_lqc->k_min_for_pk = 1.e-1;
  //ppt_lqc->k_max_for_pk = 1.e5;
   // ppt->k_min_for_pk = 1.e2*a_prime_over_a_ini;
   ppt_lqc->k_max_for_pk = 1.e1;
   ppt_lqc->k_min_for_pk = 1.e-7;
  pba_lqc->k_min_for_pk = 1.e-7*pba_lqc->a_bounce*sqrt(3.*pba_lqc->rho_bounce);
...
int input_default_precision ( struct precision * ppr ) {
  ...
  ppr->back_lqc_integration_stepsize = 1.e-1;
  ppr->tol_background_lqc_integration = 1.e-4;


  ppr->perturb_lqc_integration_stepsize = 1.e-1;
  ppr->tol_perturb_lqc_integration = 1.e-4;
  ppr->perturb_lqc_sampling_stepsize=1.0;
  ...
  ppr->tol_perturb_integration=1.e-2;
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  struct background_lqc ba_lqc;       /* for cosmological background */
  struct perturbs_lqc pt_lqc;         /* for source functions */
